...
func TestStatsWriter(t *testing.T) {
	t.Run("ok", func(t *testing.T) {
		...
		for _, p := range srv.Payloads() {
			...
			assert.Contains(testSets, statsPayload.Stats)
		}
	})
	...
	t.Run("buildPayloads", func(t *testing.T) {
		t.Run("ok", func(t *testing.T) {
			...
			actualBuckets := make([]pb.ClientStatsBucket, 0)
			for i := 0; i < expectedNbPayloads; i++ {
				assert.Equal(1, len(payloads[i].Stats))
				actualBuckets = append(actualBuckets, payloads[i].Stats[0])
				assert.Equal(expectedNbEntriesByPayload[i], len(payloads[i].Stats[0].Stats))
			}
			assert.Equal(extractCounts(stats), extractCounts(actualBuckets))
		})
...
func testStatsWriter() (*StatsWriter, chan []pb.ClientStatsBucket, *testServer) {
	srv := newTestServer()
	// We use a blocking channel to make sure that sends get received on the
	// other end.
	in := make(chan []pb.ClientStatsBucket)
...
func extractCounts(stats []pb.ClientStatsBucket) map[key]counts {
